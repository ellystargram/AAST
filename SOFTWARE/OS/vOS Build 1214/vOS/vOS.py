...
GPIO.setup([X_OVR, Y_OVR, X_PLUS, X_MINUS, Y_PLUS, Y_MINUS], GPIO.OUT)
...
while True:
        ...
        if ((scores[i] > minConfidenceThreshold) and (scores[i] <= 1.0)):
            ...
            if (objectName == 'zebra'):
                ...
                detectedObjects.append({
                    ...
                    'label': objectName,
                    'score': scores[i],
                    'xy': (xTarget, yTarget, xPoint, yPoint),
                    'distance': distance
                })
    ...
    # 거리에 대한 오름차순 정렬
    detectedObjects = sorted(detectedObjects, key=lambda x: x['distance'], reverse=False)

    if detectedObjects:
        largest_object = detectedObjects[0]

        # 객체 정보 담기
        xMin, yMin, xMax, yMax = largest_object['box']
        objectName = largest_object['label']
        score = largest_object['score']
        xTarget, yTarget, xPoint, yPoint = largest_object['xy']
        distance = largest_object['distance']

        cv2.line(frame, (int(xTarget), int(yTarget)), (int(xFrame / 2), int(yFrame / 2)), (0, 255, 0), 1, cv2.LINE_AA)
    
        if xPoint > 0:
            GPIO.output(X_MINUS, GPIO.LOW)
            GPIO.output(X_PLUS, GPIO.HIGH)
            print('x+')
            
        elif xPoint < 0:
            GPIO.output(X_PLUS, GPIO.LOW)
            GPIO.output(X_MINUS, GPIO.HIGH)
            print('x-')
            
        if yPoint > 0:
            GPIO.output(Y_MINUS, GPIO.LOW)
            GPIO.output(Y_PLUS, GPIO.HIGH)
            print('y+')
            
        elif yPoint < 0:
            GPIO.output(Y_PLUS, GPIO.LOW)
            GPIO.output(Y_MINUS, GPIO.HIGH)
            print('y-')
    
    elif not detectedObjects:
        GPIO.output(X_PLUS, GPIO.LOW)
        GPIO.output(X_MINUS, GPIO.LOW)
        GPIO.output(Y_PLUS, GPIO.LOW)
        GPIO.output(Y_MINUS, GPIO.LOW)
